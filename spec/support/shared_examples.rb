shared_examples 'calls common resources' do
  it 'installs and configures common Rack app dependencies' do
...
shared_examples 'default recipe behavior' do
  include_examples 'calls common resources'
...
shared_examples 'runs app with Chef embedded ruby' do
  include_examples 'calls common resources'
  ...
  it 'sets run_state local vars correctly in the lwrp action' do
    expect(chef_run.node.run_state['passenger']).to eq(
      'attributes-app' => {
        ...
        'bundle_bin' => '/opt/chef/embedded/bin/bundle'
      }
    )
  end
  ...
  "user": "attributes-app"
    ...
    expect(chef_run).not_to install_ruby_build_ruby('attributes-app ruby')
    ...
    expect(chef_run).to run_execute(
      '/opt/chef/embedded/bin/bundle install --deployment --without development test'
    ).with(
      cwd: '/opt/passenger/attributes-app',
      user: 'attributes-app',
      group: 'passenger'
    ...
    expect(restart_execute.cwd).to eq('/opt/passenger/attributes-app')
    expect(restart_execute.command).to eq(
      '/opt/chef/embedded/bin/bundle exec passenger-config restart-app ' \
        '/opt/passenger/attributes-app'
    )
    expect(restart_execute).to do_nothing
    expect(restart_execute).to subscribe_to('git[attributes-app]').on(:run).delayed
    ...
    expect(stop_execute.cwd).to eq('/opt/passenger/attributes-app')
    expect(stop_execute.command).to eq(
      '/opt/chef/embedded/bin/bundle exec passenger stop'
    )
    expect(stop_execute).to do_nothing
    expect(stop_execute).to notify('execute[start attributes-app]').to(:run).delayed
    ...
    expect(chef_run).to run_execute('start attributes-app').with(
      cwd: '/opt/passenger/attributes-app',
      command: '/opt/chef/embedded/bin/bundle exec passenger start'
    )
  end
end
